"""PyBulletPandaEnv._resolve_renderer 单元测试（iter2-renderer-env-mode）。
...
def test_resolve_renderer_cpu_returns_tiny():
    ...
    assert PyBulletPandaEnv._resolve_renderer("cpu") == p.ER_TINY_RENDERER


def test_resolve_renderer_gpu_returns_opengl():
    """'gpu' → p.ER_BULLET_HARDWARE_OPENGL。"""
    assert PyBulletPandaEnv._resolve_renderer("gpu") == p.ER_BULLET_HARDWARE_OPENGL
...
def test_resolve_renderer_auto_apple_silicon():
    ...
    with patch("env.pybullet_env.platform") as mock_platform:
        ...
        assert PyBulletPandaEnv._resolve_renderer("auto") == p.ER_TINY_RENDERER
...
def test_resolve_renderer_auto_linux_x86():
    ...
    with patch("env.pybullet_env.platform") as mock_platform:
        ...
        assert PyBulletPandaEnv._resolve_renderer("auto") == p.ER_BULLET_HARDWARE_OPENGL
...
def test_resolve_renderer_invalid_value_raises(bad_value: str):
    ...
    with pytest.raises(ValueError) as exc_info:
        PyBulletPandaEnv._resolve_renderer(bad_value)
