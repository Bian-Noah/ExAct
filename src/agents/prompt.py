...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "    - **operation='vla'**（默认）：执行 VLA 推理。**指令的格式、长度、语言要求以额外提示词段为准**；不符合时工具会拒绝并返回原因，按原因修正后重试。\n"
    "    **遇到不确定的目标**（如不确定物体颜色/位置/可达性）时，**先调用 explore.write_note 记录你的判断**，"
    "再根据动作执行结果（ee_pos 变化、动作返回值）调整下一条指令。\n"
    ...
    "reset 路径不消耗指令规范校验，instruction 可省略（默认空）。\n"
    ...
    "**遇到拿不准的情况**：先用 explore.write_note 写出你的判断（例如 '这条 VLA 后端对该画面的反应可能不准'），再决定下一步 action 指令。\n"
    "请按以下流程完成任务：\n"
    "1. 首先调用一次 observe 工具（不传 target），获取场景的 RGB 图像与状态描述。\n"
    "2. 仔细查看返回的图像，识别物体位置、颜色、形状以及与机械臂末端的相对关系。\n"
    "3. 基于图像与文本观察结果，规划下一步动作并调用 action 工具（**指令风格与禁区见额外提示词段**）；"
    "如果不确定 VLA 能否完成或目标在哪，先调 explore.write_note 记录试探，再发 action。\n"
